...
<template>
  <div class="c-user">
    ...
    <span class="c-user__username u-text-ellipsis">{{
      username.length == 42 ? truncateAddress(username) : `@${username}`
    }}</span>
    <button
      v-if="isFollowing"
      class="c-user__action"
      type="button"
      @click="handleUnfollow(owner)"
    >
      Following
    </button>
    <button
