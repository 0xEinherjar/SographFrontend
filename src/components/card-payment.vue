...
async function subscription() {
  ...
  const result = await blockchain.subscription();
  if (result.success) {
        ...
        <div class="c-card-payment__count u-flex-line">
          <button class="c-card-payment__count-button" @click="counterDuration('-')">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none">
              <path fill="#28292b" fill-rule="evenodd" d="M0 12a1.5 1.5 0 0 1 1.5-1.5h21a1.5 1.5 0 0 1 0 3h-21A1.5 1.5 0 0 1 0 12" clip-rule="evenodd"></path>
            </svg>
          </button>
          <span class="c-card-payment__count-text">{{ duration }} year{{ duration == 1 ? "" : "s" }}</span>
          <button class="c-card-payment__count-button" @click="counterDuration('+')">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none">
              <path fill="#28292b" fill-rule="evenodd" d="M13.5 1.5a1.5 1.5 0 0 0-3 0v9h-9a1.5 1.5 0 0 0 0 3h9v9a1.5 1.5 0 0 0 3 0v-9h9a1.5 1.5 0 0 0 0-3h-9z" clip-rule="evenodd"></path>
            </svg>
          </button>
        </div>
        <button class="c-card-payment__pay" @click="subscription" type="button">
          Subscribe for {{ price }} USDT per {{ duration }} year{{ duration == 1 ? "" : "s" }}
        </button>
      </div>
    </div>
  </dialog>
  <div @click="modal.showModal()">Subscription</div>
</template>
<style>
